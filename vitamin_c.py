...
"""
params = '/home/chrism/vitamin_c/params.json'
bounds = '/home/chrism/vitamin_c//bounds.json'
fixed_vals = '/home/chrism/vitamin_c/fixed_vals.json'
run = time.ctime().replace(' ', '-')
EPS = 1e-6

path = os.path.join('/data/www.astro/chrism/vitamin_c', run) 
os.mkdir(path) 
...
idx_mask = np.argsort(m1_idx_mask + m2_idx_mask + gauss_idx_mask + vonmise_idx_mask)
...
not_xyz_mask, not_xyz_idx_mask, not_xyz_len = get_param_index(params['inf_pars'],['mass_1','mass_2','psi','phase','geocent_time','theta_jn'])
...
#        pool_strides_r1 = self.params['pool_strides_r1']
        ...
        """
        a2 = tf.keras.layers.Dense(4096,activation='relu')(a)
        a2 = tf.keras.layers.Dense(2048,activation='relu')(a2)
...
#        self.encoder_q = tf.keras.Model(inputs=[q_input_y, q_input_x], outputs=e)
        ...
        c = tf.keras.layers.Flatten()(q_input_x)
        d = tf.keras.layers.concatenate([a,c])
        e = tf.keras.layers.Dense(4096,activation='relu')(d)
        e = tf.keras.layers.Dense(2048,activation='relu')(e)
...
#        self.decoder_r2 = tf.keras.Model(inputs=[r2_input_y, r2_input_z], outputs=l)
        ...
        g = tf.keras.layers.Flatten()(r2_input_z)
        h = tf.keras.layers.concatenate([a,g])
        i = tf.keras.layers.Dense(4096,activation='relu')(h)
        i = tf.keras.layers.Dense(2048,activation='relu')(i)
...
def compute_loss(model, x, y, y_normscale=1.0, ramp=1.0, noiseamp=1.0):
...
def gen_samples(model, y, nsamples=1000):
...
@tf.function
def train_step(model, x, y, optimizer, y_normscale, ramp):
...
#    x_data_test = x_data_test.astype(np.float32)
    ...
    # plot y training examples
#    for i in range(10):
#        print(i)
#        plt.plot(np.random.normal(0,1,size=1024))
#        plt.plot(y_data_val[i,:,0])
#        plt.savefig('/home/hunter.gabbard/public_html/random_stuff/val_set_fix/val_%d.png' % i)
#        plt.close()
#    exit()
    ...
    for epoch in range(1, epochs + 1):
        ...
        for step, (x_batch_train, y_batch_train) in train_dataset.enumerate():
            temp_train_r_loss, temp_train_kl_loss = train_step(model, x_batch_train, y_batch_train, optimizer, params['y_normscale'], ramp=ramp)
        ...
        for step, (x_batch_val, y_batch_val) in val_dataset.enumerate():
            temp_val_r_loss, temp_val_kl_loss = compute_loss(model, x_batch_val, y_batch_val, y_normscale=params['y_normscale'], ramp=ramp)
            val_loss[epoch-1,0] += temp_val_r_loss
            val_loss[epoch-1,1] += temp_val_kl_loss
        val_loss[epoch-1,:] /= float(step)
        end_time_val = time.time()

        print('Epoch: {}, Training RECON: {}, KL: {}, TOTAL: {}, time elapsed: {}'
            .format(epoch, train_loss[epoch-1,0], train_loss[epoch-1,1], train_loss[epoch-1,0] + ramp*train_loss[epoch-1,1], end_time_train - start_time_train))
        print('Epoch: {}, Validation RECON: {}, KL: {}, TOTAL: {}, time elapsed {}'
            .format(epoch, val_loss[epoch-1,0], val_loss[epoch-1,1], val_loss[epoch-1,0] + ramp*val_loss[epoch-1,1], end_time_val - start_time_val))
        ...
        if epoch % 10 == 0: # 10
            for step, (x_batch_test, y_batch_test) in test_dataset.enumerate():             
                ...
                samples = gen_samples(model, y_batch_test, nsamples=8000)
            ...
            # plot KL evolution
            plot_KL(np.reshape(np.array(KL_samples),[-1,params['r']]),250,run=plot_dir)
    ...
    return val_loss[epoch-1,0] + ramp*val_loss[epoch-1,1]
